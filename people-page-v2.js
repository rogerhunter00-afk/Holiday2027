(() => {
  ...
  async function syncMe() {
    ...
    // Keep People and the main feed on the same canonical shared board.
    await syncMainBoard(mine);
    if (mine.length) {
  ...
  async function writeMainBoard(action, payload = {}) {
    const { session } = await getClientAndSession();
    const me = localProfile();
    ...
    if (!r.ok) throw new Error(data.error || ('Shared board write returned ' + r.status));
    return data;
  }

  async function syncMainBoard(mine) {
    for (const o of mine) {
      const row = {
        id:o.id,
        ...
        guests_count:numberFrom(o.guests),
        guests_text:o.guests || null,
        total_price:numberFrom(o.price),
        currency:currencyFrom(o.price || o.perPerson),
        per_person_price:numberFrom(o.perPerson),
      ...
      await writeMainBoard('upsert_option', { option:row });
      if (o.voted) await writeMainBoard('set_vote', { option_id:o.id, voted:true });
    }
  }
